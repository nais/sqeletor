			...
			When("sql instance exists and is ready", func() {
				...
				When("no secret exists", func() {
					...
					It("should create a secret containing the env vars", func() {
						...
						Expect(secret.StringData).To(HaveKeyWithValue(envVarPrefix+"_SSLKEY_PK1_PEM", "/var/run/secrets/nais.io/sqlcertificate/key.pk1.pem"))
						Expect(secret.StringData).To(HaveKeyWithValue(envVarPrefix+"_SSLKEY_PK8", "/var/run/secrets/nais.io/sqlcertificate/key.pk8"))
						Expect(secret.StringData).To(HaveKeyWithValue(envVarPrefix+"_SSLKEY_PK8_DER", "/var/run/secrets/nais.io/sqlcertificate/key.pk8.der"))
						Expect(secret.StringData).To(HaveKeyWithValue(envVarPrefix+"_SSLKEY_PK8_PEM", "/var/run/secrets/nais.io/sqlcertificate/key.pk8.pem"))
