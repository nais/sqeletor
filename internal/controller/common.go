...
func validateOwnership(ownerReference meta_v1.OwnerReference, meta meta_v1.Object) error {
	...
	if meta.GetLabels()[managedByKey] != sqeletorFqdnId {
		return fmt.Errorf("secret %s in namespace %s is not managed by us: %w", meta.GetName(), meta.GetNamespace(), errNotManaged)
	}

	ownerReferences := meta.GetOwnerReferences()
	if len(ownerReferences) > 1 {
		return fmt.Errorf("secret %s in namespace %s has multiple owner references: %w", meta.GetName(), meta.GetNamespace(), errMultipleOwners)
	}
	...
	if ownerReferences[0].APIVersion != ownerReference.APIVersion ||
		...
		return fmt.Errorf("secret %s in namespace %s has different owner reference: %w", meta.GetName(), meta.GetNamespace(), errOwnedByOther)
	}
